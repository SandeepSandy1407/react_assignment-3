...
class MainComponent extends Component {
  ...
  startComponent = () => {
    ...
    const temId = imagesList[randNum]
    this.setState({
      selectedImageId: temId.id,
      isRunning: true,
      timerVal: 60,
      score: 0,
    })
  ...
  timerFunction = () => {
    ...
    if (timerVal > 0) {
      this.setState(prevState => ({timerVal: prevState.timerVal - 1}))
    } else if (timerVal <= 0) {
      this.lossCardActivate()
    }
  }
  ...
  lossCardComponent = () => {
    const {timerVal, score} = this.state
  ...
  runningApiState = () => {
    const {timerVal, score, selectedTabId, selectedImageId} = this.state
    const selectedImageUrl = imagesList.find(
      eachImage => eachImage.id === selectedImageId,
    )
    const filteredListItems = imagesList.filter(
    ...
    return (
      <>
        ...
        <div>
          <div>
            <img src={selectedImageUrl.imageUrl} alt="match" />
          </div>
          <ul>
          ...
          <ul>
            {filteredListItems.map(eachImageItem => (
              <ImageListItems
                key={eachImageItem.id}
                eachImage={eachImageItem}
  ...
  render() {
    ...
    const finalComponent = isRunning
      ? this.runningApiState()
      : this.lossCardComponent()
